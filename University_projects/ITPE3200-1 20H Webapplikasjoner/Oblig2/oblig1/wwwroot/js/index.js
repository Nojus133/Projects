...
function byggBillettTyper() {
    $.get("Bestilling/GetPris", function(data){
        formatterBillettTyper(data);
    }).fail(function(err){

    });
}

const handleBillettTypeEvent = (function() {
        let teller = 1;
        return function(e, that, value) {
            teller = value || teller;
            if (value === 0) teller = 0;
            if (e === undefined && that === undefined) return;
            let input = that.children[1];
                
            if (e.target.type === "button") {
                if (e.target.textContent === "-") {
                    if (teller === 9 && input.value === "0") {
                        return;
                    }
...
function formatterBillettTyper(data) {
    const container = document.querySelector(".input-div.reisende");
    
    data.forEach((obj, index) => {
        const parentDiv = document.createElement("div");
        const value = obj.type;

        parentDiv.classList.add("en-type");

        const btnMinus = document.createElement("button");
        btnMinus.setAttribute('type', 'button');
        btnMinus.setAttribute('tabindex', '-1');
        btnMinus.classList.add("btn2");
        ...
        input.addEventListener("keydown", function(e){
            const regex = /[^0-9]/;
            const isNotNumber = regex.test(e.key);
            const keyVal = Number(e.key);
            const alleDivs = Array.from(this.parentElement.parentElement.children)
            .filter(e => e.className === "en-type")
            const inputs = Array.from(alleDivs.map(e=> e.children))
            .map(input => {
                return Array.from(input).filter(i => i.localName === "input");
            });
            const values = inputs.map(input => input[0])
            .map(input => input.value)
            .map(val => Number(val))
            .reduce((acc, curr)=> acc + curr);
            console.log(e.key);
            const totalVal = keyVal + values;
            if (e.key === 'Backspace' || e.key === 'Delete' || e.key === 'Tab' ||
                e.key === "ArrowUp" || e.key === "ArrowDown") return;
            if (isNotNumber || totalVal > 9) e.preventDefault();
            else if (keyVal !== 0) {
        ...
        input.addEventListener("focusout", function(){
            ...
            handleBillettTypeEvent(undefined, undefined, values);
        })

        const btnPlus = document.createElement("button");
        btnPlus.setAttribute('type', 'button');
        btnPlus.setAttribute('tabindex', '-1');
        btnPlus.classList.add("btn2");
        btnPlus.textContent = "+";

        const label = document.createElement("label");
        label.setAttribute("for", input.id);
        label.textContent = storBokstav(value);

        if (value === "barn") {
            label.textContent += " (6 - 17 år)";
        }
        if (index === 0) {
        ...
        parentDiv.addEventListener("click", function(e){
            if (e.target.type === "button") {
                handleBillettTypeEvent(e, this);
            }
        });
        parentDiv.appendChild(btnMinus);
        parentDiv.appendChild(input);
        parentDiv.appendChild(btnPlus);
...
function hentSteder() {
    ...
    .fail(function(err){
        $(".error").text(handleError(err, "Kunne ikke hente strekninger."));
    });
    return stederArray;
}
...
function validerInputs() {
    ...
    let antall = 0;
    const antallInput = Array.from(document.querySelectorAll(".en-type input"));
...
function finnBillett(ruter, bestilling) {
    ...
    .fail(function(err){
        $("#err").text(handleError(err, "Kunne ikke hente tidene."));
    });
}
